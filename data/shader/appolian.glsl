...
float weird(vec2 p) {
    float z = 4.0;
    p *= ROT(TIME*0.25*3.14159);
    float tm = 0.125*TIME*3.14159;
    float r = 0.5;
...
vec3 color(vec2 p) {
    ...
    float l   = length(p);
    float hue = fract(0.75*l-0.25*TIME)+0.3+0.15;
...
vec3 postProcess(vec3 col, vec2 q)  {
    ...
    col*=0.5+0.5*pow(19.0*q.x*q.y*(1.0-q.x)*(1.0-q.y)*peak, 0.7);// vigneting
    return col;
}
