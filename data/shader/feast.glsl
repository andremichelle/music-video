...
{//Technique is to make a new position np and tweak it, clone it, rotate it and then pass np to fb to create complex geometry from simple piece
    b=sin(p.z-tt*16.)*0.1;//animation variable based on sin of z axis
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    tt=mod(iTime,64.)*6.2831853072*0.25;//MAin time variable, it's modulo'ed to avoid ugly artifact. Holding time in my hand: playing god is nearly as good as this crystal meth bag
    ...
    if(t>0.){//If t>0 then we must have hit some geometry so let's fucking shade it. 
        ...
        if(sc.y>5.) al=vec3(rPeak);//albedo becomes white
        ...
        if(sc.y>8.) {al=vec3(lPeak);}//Anyway it's easy way of redefining materials to include the gloss map based on bp rather than np as these materials are all for outter bits
    ...
    co+=g1*lPeak;//Adding the white gamma ray glow at the end
