/*
https://www.shadertoy.com/view/WtfSDf
...
void cam(inout vec3 p) {
    float t=time*0.125 + curve(time, 3.7, 10.0)*5.0;
    float t2=time*0.2 + curve(time, 2.7, 10.0)*3.0;
    p.xz *= rot(t);
    p.xy *= rot(t2);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 atmo = mix(vec3(1, 0.7, 0.3), vec3(0.2, 1.0, 0.6), pow(abs(r.x)*fa,3.0));
    atmo = mix(atmo, vec3(0.5, 0.7, 1.4)*3.0, pow(abs(r.y)*fb,7.0));
    ...
    #if 1
    float t3 = time*8.0 - length(uv)*0.2;
    col.xy *= rot(t3);
    col.xz *= rot(t3*0.7);
    col=abs(col);
