...
void mainImage( out vec4 fragColor, in vec2 fragCoord )//2 lines above are a = ambient occlusion and s = sub surface scattering
{
    ...
    tt=mod(iTime*0.392699,62.8318);  //Time variable, modulo'ed to avoid ugly artifact. Imagine moduloing your timeline, you would become a cry baby straight after dying a bitter old man. Christ, that's some fucking life you've lived, Steve.
    vec3 ro=mix(vec3(1),vec3(-0.5,1,-1),ceil(sin(tt)))*vec3(10,2.8+smoothstep(-1.0,1.0,1.5*cos(tt)),cos(tt)),//Ro=ray origin=camera position We build camera right here broski. Gotta be able to see, to peep through the keyhole.
